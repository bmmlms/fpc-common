...
type
  ...
  TStepLanguage = class(TStep)

  end;

  TStepStorage = class(TStep)

  end;

  TStepUpdates = class(TStep)
...
constructor TfrmSetupWizard.Create(AOwner: TComponent);
begin
  inherited;
  ...
  FStepList.Add(TStepLanguage.Create('Select language', pnlLanguage));
  FStepList.Add(TStepStorage.Create('Configure storage', pnlStorage));
  FStepList.Add(TStepUpdates.Create('Configure updates', pnlUpdates));

  RegisterSteps;
end;

procedure TfrmSetupWizard.Finish;
begin
...
procedure TfrmSetupWizard.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;
...
procedure TfrmSetupWizard.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
 if Key = 27 then
  begin
...
procedure TfrmSetupWizard.FormShow(Sender: TObject);
begin
  btnNext.SetFocus;
  SetText;
end;

procedure TfrmSetupWizard.InitStep(Step: TStep);
begin
...
function TfrmSetupWizard.IsValid(Step: TStep): Boolean;
begin
  Result := True;
...
procedure TfrmSetupWizard.NextStep;
begin
  if IsValid(FActiveSetup) then
    SetStep(FActiveIndex + 1);
end;

procedure TfrmSetupWizard.PrevStep;
begin
  SetStep(FActiveIndex - 1);
end;

procedure TfrmSetupWizard.RegisterSteps;
begin

end;

procedure TfrmSetupWizard.SetStep(Idx: Integer);
begin
...
procedure TfrmSetupWizard.SetText;
begin
  ...
  lblUpdates.Caption := _('Please choose if automatic search for updates should be enabled.'#13#10'No personal information will be transferred and you will be asked before an update will be downloaded or applied.'#13#10);
end;
