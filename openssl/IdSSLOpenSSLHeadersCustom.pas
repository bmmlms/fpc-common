...
unit IdSSLOpenSSLHeadersCustom;
...
//NETSCAPE_CERT_SEQUENCE
 ...
 {$EXTERNALSYM SSL_CIPHER_get_bits}
  ...
  // Custom
  SSL_get_verify_result: function(const s: PSSL): TIdC_LONG cdecl = nil;
  X509_NAME_get_index_by_NID: function(name: PX509_NAME; _nid: TIdC_INT; _lastpos: TIdC_INT): TIdC_INT; cdecl = nil;
  X509_NAME_get_entry: function(name: PX509_NAME; _loc: TIdC_INT): PX509_NAME_ENTRY; cdecl = nil;
...
//
function Load(LibEayDLLPath, SSLEayDLLPath: string): Boolean;
procedure Unload;
...
function LoadSSLLibrary(DLLPath: string): Integer;
begin
  {$IFDEF KYLIXCOMPAT}
  // Workaround that is required under Linux (changed RTLD_GLOBAL with RTLD_LAZY Note: also work with LoadLibrary())
  Result := HackLoad(DLLPath, SSLDLLVers);
...
begin
  ...
  if hIdCrypto = 0 then begin
    hIdCrypto := LoadSSLCryptoLibrary(LibEayDLLPath);
  end;
  if hIdSSL = 0 then begin
    hIdSSL := LoadSSLLibrary(SSLEayDLLPath);
  end else begin
    Exit;
  end;
  ...
  @SSL_get_verify_result := LoadFunction('SSL_get_verify_result');
